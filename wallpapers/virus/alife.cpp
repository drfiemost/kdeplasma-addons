...
void Alife::executeCell(int id)
{
    ...
    while(!stop && cell->energy > 0 && max) {
        ...
        switch(cell->code[codePointer]){
            ...
            case 4:{ //eat red
                ...
                if(color && cell->energy + color < 255) {
                    int temp = std::min(color / better_eat, color);
                    color -= temp;
                    cell->energy += temp;
            ...
            case 17:{ //kill facing
                ...
                if(temp->alive && temp->code[0] == reg) {
                    ...
                    cell->energy = std::min(255, enTemp);
                }
            }break;
            case 18:{ //give
                QPoint neighbour = getNeighbour(cell->x, cell->y, facing);
                ...
                temp->energy = std::min(255, enTemp);
                cell->energy /= 2;
            }break;
            case 19:{ //better eat
        ...
        better_eat = std::min(better_eat + 1, m_current_eat);
        max--;
    }
...
bool Alife::reproduce(struct cell* cell, int direction, QRgb color)
{
    ...
    if(!newCell->alive&& m_livingCells.size() < m_maxViruses) {
        ...
        newCell->r = std::min(r, 255);
        newCell->g = std::min(g, 255);
        newCell->b = std::min(b, 255);
...
void Alife::virusMove()
{
    ...
    m_current_eat = std::max(MAX_EAT, (int)(((double)m_livingCells.size()/ (double) (m_maxViruses / 4.0)) * MAX_EAT));
    m_current_eat_best = std::max(MIN_EAT, (int)(((double)m_livingCells.size()/ (double) (m_maxViruses / 4.0)) * (MIN_EAT * 2)));
    bool reseted = false;
